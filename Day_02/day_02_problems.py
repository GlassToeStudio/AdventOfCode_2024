...
def format_data(in_file: TextIOWrapper) -> list[list[int]]:
    """Return a list of lists of int from the given text."
    ...
    for x in in_file.readlines():
        r.append([int(y) for y in x.split()])
    return r
...
def is_safe(report):
    for i, level in enumerate(report[:-1]):
        ...
        if any(
            (increasing and next_level <= level,
             not increasing and next_level >= level,
             abs(next_level - level) > 3,
             )
        ):
            return False
    return True
