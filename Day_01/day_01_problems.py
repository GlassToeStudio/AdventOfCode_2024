...
The Chief Historian is always present for the
big Christmas sleigh launch, but nobody has seen him in months! Last anyone
heard, he was visiting locations that are historically significant to the
North Pole; a group of Senior Historians has asked you to accompany them as
...
def format_data(in_file: TextIOWrapper):  # -> tuple[list, list]:
    ...
    Returns:
        tuple[list, list]:: input data as  tuple[list, list]:
    """
...
def problem1(first, second):
    return sum(map(lambda x, y: abs(x - y), first, second))


def problem2(first, second) -> int:
    return sum(map(lambda x, _: x * second.count(x), first, second))


if __name__ == "__main__":
    with open("Day_01/input.txt", "r", encoding="utf-8") as f:
        left, right = format_data(f)
        print(problem1(left, right))
        print(problem2(left, right))
